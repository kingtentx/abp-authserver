...
namespace Cimc.Excel
{
    /// <summary>
    /// 基于 EPPlus 4.5.3.3 插件
    /// </summary>
    public class ExcelHelper
    {
                    ...
                    foreach (var en in dataList)
                    {
                        for (int i = 0; i < keys.Count; i++) // 根据指定的属性名称，获取对象指定属性的值
                        {
                            ...
                            if (properotyInfo != null)
                            {
                                ...
                                var t = properotyInfo.PropertyType;
                                if (t == typeof(DateTime) || t == typeof(DateTime?))
                                {
                                    if (properotyValue != null)
                                        cellValue = Convert.ToDateTime(properotyValue).ToString("yyyy-MM-dd HH:mm:ss");
                                    else
                                        cellValue = "";
                                }
                                else
                                {
                                    cellValue = properotyValue == null ? "": properotyValue.ToString();
                                }
                            }
                            //// 3.3 属性值经过转换赋值给单元格值
                            //if (properotyValue != null)
        ...
        public static (bool Success, string Message, List<T> List) ExcelToList<T>(Dictionary<string, string> cellHeard, string uploadPath) where T : new()
        {
            try
            {
                ...
                return (true, errorMsg.ToString(), enlist);
            }
            catch (Exception ex)
            {
